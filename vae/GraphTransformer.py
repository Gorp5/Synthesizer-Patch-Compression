...
'''
from doctr.models import reparameterize
...
class Transformer(nn.Module):
    ...
    def forward(self, x, alg_film=None, global_film=None, return_attn_loss=False):
        alibi_bias = False
...
class GraphTransformerAutoencoder(nn.Module):
    def __init__(
        ...
        self.algorithm_embedding = nn.Embedding(
            num_embeddings=num_algorithms,
        ...
        self.enc_alg_film_gamma = nn.Sequential(nn.Linear(d_model, d_model), nn.SiLU())
        self.enc_alg_film_beta = nn.Sequential(nn.Linear(d_model, d_model), nn.SiLU())
        ...
        self.dec_alg_film_gamma = nn.Sequential(nn.Linear(d_model, d_model), nn.SiLU())
        self.dec_alg_film_beta = nn.Sequential(nn.Linear(d_model, d_model), nn.SiLU())
    ...
    def forward(self, x, return_attn_loss=True):
        ...
        algo_emb_enc = self.algorithm_embedding(algorithm_id)
        f_gamma_enc = self.enc_alg_film_gamma(algo_emb_enc)
        f_beta_enc = self.enc_alg_film_beta(algo_emb_enc)
        alg_film_enc = (f_gamma_enc, f_beta_enc)
        ...
        x += self.learned_embeddings

        if return_attn_loss:
            x, sparsity_loss_enc = self.encoder(
                x, alg_film=alg_film_enc, global_film=global_film_enc, return_attn_loss=True
            )
        else:
            x = self.encoder(x, alg_film=alg_film_enc, global_film=global_film_enc,)
        ...
        #alg_probs = torch.softmax(alg_logits, dim=-1)
        algo_emb_dec = alg_probs @ self.algorithm_embedding.weight
        f_gamma_dec = self.dec_alg_film_gamma(algo_emb_dec)
        f_beta_dec = self.dec_alg_film_beta(algo_emb_dec)
        alg_film_dec = (f_gamma_dec, f_beta_dec)
        ...
        if return_attn_loss:
            x, sparsity_loss_dec = self.decoder(
                x, alg_film=alg_film_dec, global_film=global_film_dec, return_attn_loss=True
            )
        else:
            x = self.decoder(x, alg_film=alg_film_dec, global_film=global_film_dec)
        ...
        return x_recon, latent, logvar, alg_loss, alg_probs.argmax(dim=1)
